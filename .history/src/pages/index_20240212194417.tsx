          ...
          {forcast && forcast.list && (
            <div className="flex flex-row w-full px-12 mt-12">
              ...
              {forcast.list.slice(0,5).map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-xl">Temp: {d.main.temp.toFixed(1)}</h1>
                  {d.weather.map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-xl">Weather: {d.main}</h1>
              ...
              {d.weather.map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-xl">Description: {d.description}</h1>
                </div>
              ))}
                <h1 className="flex flex-start text-xl">Wind Speed: {d.wind.speed}</h1>
                <h1 className="flex flex-start text-xl mt-12">Date: {formatDate(d.dt)}</h1>
                </div>
              ))}
