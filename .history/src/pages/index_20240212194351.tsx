      ...
      <div className="flex flex-col w-5xl justify-between w-full items-center  text-sm ">
        ...
        <div className="flex flex-col justify-center text-center w-full items-center">
          ...
          {forcast && forcast.list && (
            <div className="flex flex-row w-full px-12 mt-12">
              <h1 className="flex flex-start text-2xl">5 Day Forecast</h1>
              {forcast.list.slice(0,5).map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-2xl">Temp: {d.main.temp.toFixed(1)}</h1>
                  {d.weather.map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-2xl">Weather: {d.main}</h1>
              ...
              {d.weather.map((d, index)=> (
                <div key={index}>
                  <h1 className="flex flex-start text-2xl">Description: {d.description}</h1>
                </div>
              ))}
                <h1 className="flex flex-start text-2xl">Wind Speed: {d.wind.speed}</h1>
                <h1 className="flex flex-start text-2xl mt-12">Date: {formatDate(d.dt)}</h1>
                </div>
              ))}
